...
close all
if ispc == 1
    path = 'W:\Students\Arne\AOC\data\automagic_no_opticat';
else
    path = '/Volumes/methlab/Students/Arne/AOC/data/automagic_no_opticat/';
end
dirs = dir(path);
...
for subjects = 1 : length(subjectIDs)
    ...
    if isempty(dir(['/Volumes/methlab/Students/Arne/AOC/data/merged_no_opticat/', char(subjectID), filesep, char(subjectID), '*_merged.mat']))
        % Set up data paths
        if ispc == 1
            filePathET = ['V:\OCC\AOC\data\', char(subjectID)];
            filePathEEG = ['W:\Students\Arne\AOC\data\automagic_no_opticat\',  char(subjectID)];
            resultFolder = ['W:\Students\Arne\AOC\data\merged_no_opticat\', char(subjectID)];
        else
            filePathET = ['/Volumes/methlab_data/OCC/AOC/data/', char(subjectID)];
            filePathEEG = ['/Volumes/methlab/Students/Arne/AOC/data/automagic_no_opticat/',  char(subjectID)];
            resultFolder = ['/Volumes/methlab/Students/Arne/AOC/data/merged_no_opticat/', char(subjectID)];
        end
        mkdir(resultFolder)
        ...
        for files = [size(dEEG, 1), 1 : size(dEEG, 1)-1]
            ...
            try
                ...
                elseif strcmp(task, 'AOC_Sternberg') == 1
                    fileName = [char(subjectID) '_EEG_ET_Sternberg_block' num2str(block) '_merged_no_opticat'];
                elseif strcmp(task, 'AOC_Nback') == 1
                    fileName = [char(subjectID) '_EEG_ET_Nback_block' num2str(block) '_merged_no_opticat'];
                end
