%% AOC Master Matrix LONG — N-Back (Subject-Level)
% Extends the standard merged_data_nback with:
...
%
% Key outputs:
%   merged_data_nback_LONG.mat  (struct with original + new columns)
%   merged_data_nback_LONG.csv  (same, as table)
%
% New columns (25 → 41):
...
% Behavioral
load(fullfile(featPath, 'behavioral_matrix_nback.mat'));  % behav_data_nback

% EEG (original: AlphaPower, IAF, Lateralization)
load(fullfile(featPath, 'eeg_matrix_nback.mat'));         % eeg_data_nback

% Gaze (original: subject-level, non-baselined)
load(fullfile(featPath, 'gaze_matrix_nback.mat'));        % gaze_data_nback

% Gaze (trial-level with baselined metrics)
load(fullfile(featPath, 'gaze_matrix_nback_trials.mat')); % gaze_data_nback_trials
...
%% Save as .mat
save(fullfile(featPath, 'merged_data_nback_LONG.mat'), 'merged_data_nback_LONG');
fprintf('Saved merged_data_nback_LONG.mat\n');

%% Save as .csv
merged_table = struct2table(merged_data_nback_LONG);
writetable(merged_table, fullfile(featPath, 'merged_data_nback_LONG.csv'));
fprintf('Saved merged_data_nback_LONG.csv\n');

%% Summary
nCols = numel(fieldnames(merged_data_nback_LONG));
fprintf('\n=== N-Back LONG merge complete ===\n');
fprintf('Rows: %d  |  Columns: %d\n', numel(merged_data_nback_LONG), nCols);
