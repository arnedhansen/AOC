...
% Main subject-level master matrix. Builds merged_data_sternberg with:
...
%   Baselined raw alpha → aggregate eeg_matrix_sternberg_trials.mat
%
% Key outputs:
%   merged_data_sternberg.mat  (struct with original + new columns)
%   merged_data_sternberg.csv  (same, as table)
%
% Columns include:
...
eegTrlIDs   = [eeg_data_sternberg_trials.ID];
eegTrlConds = [eeg_data_sternberg_trials.Condition];
...
for s = 1:numel(uIDs)
    ...
    for c = 1:3
        ...
        trlSubset = eeg_data_sternberg_trials(trlMask);
...
merged_data_sternberg = struct( ...
    ...
    'BCEA',                    {gaze_data_sternberg.BCEA}, ...
    'BCEALateralization',      {gaze_data_sternberg.BCEALateralization}, ...
