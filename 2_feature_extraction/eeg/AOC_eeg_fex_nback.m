...
%       Requires tfr_nback.mat from AOC_eeg_fex_nback_TFR.m. Differs from IAF-tuned AlphaPower_bl_*.
...
for subj = 1:length(subjects)
    try
        ...
        ERSD_late = nan(3, 1);
        ERSD_full = nan(3, 1);
        tfrMatPath = fullfile(datapath, 'tfr_nback.mat');
        if isfile(tfrMatPath)
            occ_from_pow = {};
            for ii = 1:numel(pow1_raw_full.label)
                lab = pow1_raw_full.label{ii};
                if contains(lab, {'O'}) || contains(lab, {'I'})
                    occ_from_pow{end+1} = lab;
                end
            end
            S_tfr = load(tfrMatPath, 'tfr1_bl', 'tfr2_bl', 'tfr3_bl');
            if isfield(S_tfr, 'tfr1_bl') && isfield(S_tfr, 'tfr2_bl') && isfield(S_tfr, 'tfr3_bl')
                tfPack = {S_tfr.tfr1_bl, S_tfr.tfr2_bl, S_tfr.tfr3_bl};
