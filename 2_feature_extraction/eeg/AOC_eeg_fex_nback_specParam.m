...
for subj = 1:length(subjects)
    try
        ...
        condTrials = {ind1, ind2, ind3};
        condVals = [1; 2; 3];
        condNames = {'1back', '2back', '3back'};
        ...
        pow1_fooof_full = remove_time_dimension(select_data(window.full, freq_range, tfr_fooof{1}));
        pow2_fooof_full = remove_time_dimension(select_data(window.full, freq_range, tfr_fooof{2}));
        pow3_fooof_full = remove_time_dimension(select_data(window.full, freq_range, tfr_fooof{3}));
        pow1_fooof_bl_full = remove_time_dimension(select_data(window.full, freq_range, tfr_fooof_bl{1}));
        pow2_fooof_bl_full = remove_time_dimension(select_data(window.full, freq_range, tfr_fooof_bl{2}));
        pow3_fooof_bl_full = remove_time_dimension(select_data(window.full, freq_range, tfr_fooof_bl{3}));
        pow1_fooof_bl_early = remove_time_dimension(select_data(window.early, freq_range, tfr_fooof_bl{1}));
        pow2_fooof_bl_early = remove_time_dimension(select_data(window.early, freq_range, tfr_fooof_bl{2}));
        pow3_fooof_bl_early = remove_time_dimension(select_data(window.early, freq_range, tfr_fooof_bl{3}));
        pow1_fooof_bl_late = remove_time_dimension(select_data(window.late, freq_range, tfr_fooof_bl{1}));
        pow2_fooof_bl_late = remove_time_dimension(select_data(window.late, freq_range, tfr_fooof_bl{2}));
        pow3_fooof_bl_late = remove_time_dimension(select_data(window.late, freq_range, tfr_fooof_bl{3}));

        occ_idx = find(ismember(pow1_fooof_full.label, occ_channels));
        if isempty(occ_idx)
            occ_idx = 1:numel(pow1_fooof_full.label);
        end

        IAF_specParam1 = iaf_specparam_fooof(dataTFR, ind1, winIAF, occ_channels, alphaRange);
        IAF_specParam2 = iaf_specparam_fooof(dataTFR, ind2, winIAF, occ_channels, alphaRange);
        IAF_specParam3 = iaf_specparam_fooof(dataTFR, ind3, winIAF, occ_channels, alphaRange);
        IAF_specParam = [IAF_specParam1; IAF_specParam2; IAF_specParam3];
        ...
        save(fullfile(datapath, 'IAF_specParam_nback.mat'), 'fooof_specparam')
        save(fullfile(datapath, 'power_nback_fooof_TFR.mat'), ...
...
save_or_merge_nback_fooof_matrix(featPath, eeg_data_nback_FOOOF);
...
function save_or_merge_nback_fooof_matrix(featPath, fooofData)
fooofMat = fullfile(featPath, 'AOC_eeg_matrix_nback_FOOOF.mat');
fooofCsv = fullfile(featPath, 'AOC_eeg_matrix_nback_FOOOF.csv');
if isempty(fooofData)
    warning('No nback FOOOF data to save.');
    return
end
Tout = dedupe_by_key(struct2table(fooofData), {'ID', 'Condition'});
eeg_data_nback_FOOOF = table2struct(Tout); %#ok<NASGU>
save(fooofMat, 'eeg_data_nback_FOOOF');
