...
eeg_data_sternberg_baselineWindow = struct( ...
    'ID', {}, 'Condition', {}, ...
...
for subj = 1:length(subjects)
    try
        clc; fprintf('[EEG BASELINEWINDOW FFT - STERNBERG] Subject %d / %d (%s)\n', ...
...
%% Save group matrices
outMat = fullfile(featPath, 'AOC_eeg_matrix_sternberg_baselineWindow.mat');
outCsv = fullfile(featPath, 'AOC_eeg_matrix_sternberg_baselineWindow.csv');
save(outMat, 'eeg_data_sternberg_baselineWindow');
writetable(struct2table(eeg_data_sternberg_baselineWindow), outCsv);
fprintf('Saved %s\n', outMat);
fprintf('Saved %s\n', outCsv);
