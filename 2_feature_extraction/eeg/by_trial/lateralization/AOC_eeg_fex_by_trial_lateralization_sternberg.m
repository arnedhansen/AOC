%% AOC EEG Feature Extraction LATERALIZED Sternberg TRIAL-BY-TRIAL
%  Takes into account lateralization from AOC_eeg_lateralization_sternberg
%
% Extracted features:
...
eeg_data_sternberg_lateralization_trials = struct('ID', {}, 'Trial', {}, 'Condition', {}, 'AlphaPower', {}, 'IAF', {}, 'Lateralisation', {});
alphaRange = [8 14];
...
for subj = 1:length(subjects)
    ...
    try
        ...
        for t = 1:nTrials
            ...
            % Channel selection depending on lateralization index
            if latIdx >= 0
                chans = right_idx;
            elseif latIdx < 0
                chans = left_idx;
            ...
            % IAF estimation
            spec = squeeze(mean(PowAll.powspctrm(t, chans, alphaIdx), 2));
            ...
            if isempty(pks)
                ...
                powerIAF = mean(squeeze(mean(PowAll.powspctrm(t, chans, IAF_range), 2)));
            ...
            % Add to struct
            eeg_data_sternberg_lateralization_trials(end+1) = struct( ...
                'ID', sid, ...
                'Trial', t, ...
                'Condition', cond - 20, ...
                'AlphaPower', powerIAF, ...
        ...
        % Save subject table
        save(fullfile(datapath, 'eeg_matrix_sternberg_subj_lateralization_trials.mat'), 'eeg_data_sternberg_lateralization_trials');
...
% Save group-level table
if ispc
    save('W:\Students\Arne\AOC\data\features\eeg_matrix_sternberg_lateralization_trials.mat', 'eeg_data_sternberg_lateralization_trials');
else
    save('/Volumes/methlab/Students/Arne/AOC/data/features/eeg_matrix_sternberg_lateralization_trials.mat', 'eeg_data_sternberg_lateralization_trials');
end
