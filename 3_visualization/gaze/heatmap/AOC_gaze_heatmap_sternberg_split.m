%% AOC Gaze Heatmap — Sternberg (Early/Late Split)
% Loads dataET_sternberg, computes heatmaps for baseline, early (0–1 s), late (1–2 s) per condition and combined. Saves figures.
%
% Key outputs:
...
for subj = 1:length(subjects)
    clc; fprintf('[GAZE HEATMAP SPLIT - STERNBERG] Building split gaze heatmaps for Subject %d / %d \n', subj, length(subjects))
...
disp('BASELININIG...')
for subj = 1 : length(subjects)
    clc; fprintf('[GAZE HEATMAP SPLIT - STERNBERG] Baselining split heatmaps for Subject %d / %d \n', subj, length(subjects))
    % EARLY
...
% Save
saveas(gcf, fullfile(figDir, 'AOC_gaze_heatmap_sternberg_stats_OVERVIEW.png'))
