...
# %% Data sources
LONG_FILES = {
    "sternberg": os.path.join(base_dir, "data/features/merged_data_sternberg_LONG.csv"),
    "nback":     os.path.join(base_dir, "data/features/merged_data_nback_LONG.csv"),
}
TRIAL_FILES = {
...
GAZE_VARS_LONG  = ["GazeDeviation", "MSRate", "ScanPathLength"]
GAZE_VARS_TRIAL = {
    "sternberg": ["GazeDeviationFull", "ScanPathLengthFull", "MSRateFull"],
    "nback":     ["GazeDeviationFull", "ScanPathLengthFull", "MSRateFull"],
}
ALPHA_VAR_TRIAL = {
    "sternberg": "AlphaPowerLate",   # retention window (1-2 s), matches registered plan
    "nback":     "AlphaPowerFull",   # 0-2 s, matches registered plan
}
...
for task in ["sternberg", "nback"]:
    ...
    for gaze_col in GAZE_VARS_TRIAL[task]:
        ...
        if res_full is None or res_red is None:
            print(f"  [{task}] {gaze_col}: model did not converge.")
            continue
        ...
        for term in final.params.index:
            ...
            log("1_TrialGLMM", task, f"Alpha~{gaze_col}*Cond", term, "beta", b)
            log("1_TrialGLMM", task, f"Alpha~{gaze_col}*Cond", term, "p", p)

        log("1_TrialGLMM", task, f"Alpha~{gaze_col}*Cond",
...
for task in ["sternberg", "nback"]:
    ...
    for gaze_col in GAZE_VARS_LONG:
...
for task in ["sternberg", "nback"]:
    ...
    for gaze_col in GAZE_VARS_TRIAL[task]:
        ...
        print(f"\n  [{task.upper()}] {gaze_col} × {alpha_col}  (collapsed)")
        ...
        log("5_TrialCorr", task, f"r({alpha_col},{gaze_col})_collapsed",
            "mean_r", "r", mean_r)
        log("5_TrialCorr", task, f"r({alpha_col},{gaze_col})_collapsed",
