# %% AOC Stats Rainclouds — Baselined Gaze + Baselined/SpecParam Alpha (Sternberg + N-Back)
...
output_dir = f"{base_dir}/figures/stats/rainclouds/baselineSpecParam"
output_dir_stats = f"{base_dir}/data/stats/baselineSpecParam"
anova_dir = f"{base_dir}/data/stats/anova/baselineSpecParam"
...
# %% Alpha (BL / SpecParam BL) ~ Gaze (BL) * Condition + (1|ID)
print("\n=== Alpha (BL/SpecParam BL) ~ Gaze (BL) * Condition mixed models (per task) ===")
...
for task in tasks:
    ...
    cols_needed = ["ID", "Condition", "AlphaPower_bl", "AlphaPower_FOOOF_bl", "GazeDeviationFullBL", "MSRateFullBL"]
    ...
    for alpha_var in ["AlphaPower_FOOOF_bl"]:
        for gaze_var in ["GazeDeviationFullBL", "MSRateFullBL"]:
            if (alpha_var not in dat.columns) or (gaze_var not in dat.columns):
                print(f" - Missing {alpha_var} or {gaze_var}. Skipping.")
                continue
        ...
        # Gaze: compare AlphaPower ~ gaze_c + Condition vs AlphaPower ~ Condition
            ...
            formula_nogaze = (
                f'{alpha_var} ~ C(Condition, Treatment(reference="{ref_level}"))'
            )
            try:
