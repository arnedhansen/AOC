...
Input:
  /Volumes/g_psyplafor_methlab$/Students/Arne/AOC/data/stats/splits/
  AOC_splitAmpRed_sternberg_stats_input.csv

This script reproduces the inferential blocks previously executed in MATLAB:
1) Group comparison (Reduction > Amplification) on subject-mean gaze deviation.
2) Group x Load analysis on gaze deviation.
3) LMM on gaze deviation: Value ~ Group * Load + (1|Subject).
4) Follow-up for Alpha and GazeDev_pct_0_2s:
   - descriptive mean/SEM by group x load
   - Group x Load analysis
   - LMM: Value ~ Group * Load (+ random slope fallback).
"""
