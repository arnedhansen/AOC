...
for i = 1:length(variables)
    ...
    % Create boxplot 
    boxplot(data.(variables{i}), data.Condition, 'Labels', {'WM load 2', 'WM load 4', 'WM load 6'}, 'Colors', 'k');
    set(gca, 'FontSize', 30); 
    ...
    title(['Sternberg ', variables{i}], "FontSize", 40);
    ...
    saveas(gcf, strcat('/Volumes/methlab/Students/Arne/AOC/figures/stats/overview/boxplots/AOC_stats_boxplots_', save_names{i}, '_sternberg.png'));
end
...
for i = 1:length(variables)
    ...
    for cond = 1:3  % Two conditions (Low Contrast = 1, High Contrast = 2)
        ...
        xjittered = cond_data.Condition + (rand(size(cond_data.(variables{i}))) - 0.5) * jitterAmount;  % Add random jitter to x-axis
        
        % Clear any previous scatter points (in case of overlaid dots)
        scatter(xjittered, cond_data.(variables{i}), 100, 'MarkerEdgeColor', 'k', ...
...
saveas(gcf, '/Volumes/methlab/Students/Arne/AOC/figures/stats/overview/boxplots/AOC_stats_overview_boxplots_sternberg.png');
...
for i = 1:length(variables)
    ...
    title(['Sternberg ', variables{i}], 'FontSize', 30);

    hold off;

    % Save individual bar plot
    saveas(gcf, strcat('/Volumes/methlab/Students/Arne/AOC/figures/stats/overview/barplots/AOC_stats_barplots_', save_names{i}, '_sternberg.png'));
...
saveas(gcf, '/Volumes/methlab/Students/Arne/AOC/figures/stats/overview/barplots/AOC_stats_overview_barplots_percentage_change_sternberg.png');
...
for cond = 1:3
    ...
    for m = 1:n_metrics
        ...
        title(['WM load ' num2str(cond*2) ': Alpha Power vs ' gaze_metric_labels{m}], 'FontSize', 18);
    ...
    sgtitle(['WM load ' num2str(cond*2) ': Association between Alpha Power and Gaze Metrics'], 'FontSize', 20);
    
    % Save the figure (adjust the save path as necessary)
    saveas(gcf, ['/Volumes/methlab/Students/Arne/AOC/figures/stats/overview/AOC_stats_overview_associations_pow_WM' num2str(cond) '.png']);
end
...
% Save
saveas(gcf, '/Volumes/methlab/Students/Arne/AOC/figures/stats/overview/AOC_stats_overview_correlation_matrix_sternberg.png');
