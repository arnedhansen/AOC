...
# I/O Dirs
input_csv = "/Volumes/g_psyplafor_methlab$/Students/Arne/AOC/data/features/merged_data_sternberg.csv"
output_dir = "/Volumes/g_psyplafor_methlab$/Students/Arne/AOC/figures/stats/rainclouds"

# Load Data
dat = pd.read_csv(input_csv)

# Ensure Condition is ordered categorical “WM load 2/4/6”
cond_series = dat["Condition"]
if np.issubdtype(cond_series.dtype, np.number):
    uniq = np.sort(pd.unique(cond_series.dropna()))
    # Handle common encodings explicitly
    if set(uniq.tolist()) == {1, 2, 3}:
        cond_map = {1: "WM load 2", 2: "WM load 4", 3: "WM load 6"}
    elif set(uniq.tolist()) == {2, 4, 6}:
        cond_map = {2: "WM load 2", 4: "WM load 4", 6: "WM load 6"}
    else:
        # Fallback: map by rank order
        labels = ["WM load 2", "WM load 4", "WM load 6"]
        cond_map = {val: labels[i] for i, val in enumerate(sorted(uniq)[:3])}
    dat["Condition"] = dat["Condition"].map(cond_map)
else:
    # Already labelled; normalise spacing/case just in case
...
for var, ttl, ylab, sname in zip(variables, titles, y_labels, save_names):
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(output_dir, f"AOC_stats_rainclouds_{sname}_sternberg_stats.png"),
        dpi=300,
        transparent=False,
