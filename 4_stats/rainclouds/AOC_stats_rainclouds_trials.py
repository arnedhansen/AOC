# %% AOC Stats Rainclouds — Combined (Sternberg + N-back), TRIAL-LEVEL STATS
#     (plots remain subject-means per Condition for readability)
...
from rainclouds_stats_helpers import (
    iqr_outlier_filter,   # unchanged
    p_to_signif           # unchanged
)
from rainclouds_helpers_mixed_glmm_trials import mixedlm_pairwise_contrasts
from rainclouds_plotting_helpers import add_stat_brackets
...
variables  = ["Accuracy", "ReactionTime", "GazeDeviationFull", "MSRateFull", "AlphaPowerEarly", "ScanPathLengthFull"]
titles     = ["Accuracy", "Reaction Time", "Gaze Deviation", "Microsaccade Rate", "Alpha Power", "Scan Path Length"]
y_labels   = ["Accuracy [%]", "Reaction Time [s]", "Gaze Deviation [px]", "Microsaccade Rate [MS/s]", "Alpha Power [\u03BCV²/Hz]", "Scan Path Length [px]"]
save_names = ["acc", "rt", "gazedev", "ms", "pow", "spl"]

# Manual ticks/limits (adapt as needed)
yticks_map = {
    ...
    "ScanPathLength": np.arange(0, 20, 2.0),
}
ylims_map = {
    "Accuracy"      : (60, 102),
...
# %% Task configurations — now pointing to *_trials.csv

tasks = [
    {
        "name"       : "sternberg",
        "input_csv"  : f"{base_dir}/data/features/merged_data_sternberg_trials.csv",
...
#     {
#         "name"       : "nback",
#         "input_csv"  : f"{base_dir}/data/features/merged_data_nback_trials.csv",
...
for _task in tasks:
    ...
    if np.issubdtype(_cond.dtype, np.number):
        ...
        _dat["Condition"] = _dat["Condition"].astype(str).str.replace(r"\s+", " ", regex=True).str_strip()

    _dat["Condition"] = pd.Categorical(_dat["Condition"], categories=_task["categories"], ordered=True)
    _dat["ID"] = _dat["ID"].astype(str)
    _dat["Trial"] = _dat["Trial"].astype(int)

    # trial-level outlier filter per Condition
    _dat = iqr_outlier_filter(_dat, variables, by="Condition")

    # global maxima for bracket ceiling
...
for task in tasks:
    ...
    # Remove impossible Accuracy values if % is present; if Accuracy is 0/1 leave as is
    if dat["Accuracy"].max() > 1.5:  # looks like percentage
    ...
    # --- Trial-level outlier removal (per Condition)
    dat = iqr_outlier_filter(dat, variables, by="Condition")

    # --- Descriptives at subject-mean level (for the figure legend/CSV)
    # Collapse to subject means per Condition (these are the points we plot)
    desc_rows = []
    subj_means = (dat
                  .groupby(["ID", "Condition"], observed=True, dropna=False)[variables]
                  .mean(numeric_only=True)
                  .reset_index())

    for var in variables:
        subdat = subj_means.loc[~subj_means[var].isna(), ["ID", "Condition", var]]
        if subdat.empty:
            continue
        ...
        for cond_lab, g in subdat.groupby("Condition", observed=True):
            ...
            iqr = q3 - q1
            desc_rows.append([task["name"], var, cond_lab, n,
                              round(mean, 2), round(sd, 2), round(sem, 2),
                              round(med, 2), round(q1, 2), round(q3, 2), round(iqr, 2)])

    desc_table = pd.DataFrame(
        desc_rows,
        columns=["Task", "Variable", "Condition", "N", "Mean", "SD", "SEM", "Median", "Q1", "Q3", "IQR"]
    )
    out_csv = os.path.join(output_dir, f"AOC_descriptives_{task['name']}_trials.csv")
    os.makedirs(output_dir, exist_ok=True)
    desc_table.to_csv(out_csv, index=False)
    print(f"Saved descriptives → {out_csv}")

    # Category order / palette mapping for plotting
    condition_order = [c for c in task["categories"] if c in subj_means["Condition"].cat.categories]
    pal_dict = dict(zip(condition_order, pal))

    # --- Loop variables
    for var, ttl, ylab, sname in zip(variables, titles, y_labels, save_names):

        # ---------- PLOTTING DATA (subject means) ----------
        dvar_plot = subj_means.loc[~subj_means[var].isna(), ["ID", "Condition", var]].copy()
        if dvar_plot.empty:
            continue
        dvar_plot["Condition"] = pd.Categorical(dvar_plot["Condition"],
                                                categories=condition_order, ordered=True)

        # ---------- STATS DATA (trials) ----------
        dvar_stats = dat.loc[~dat[var].isna(), ["ID", "Trial", "Condition", var]].copy()
        if dvar_stats.empty:
            continue
        dvar_stats["Condition"] = pd.Categorical(dvar_stats["Condition"],
                                                 categories=condition_order, ordered=True)

        # Mixed model (Gaussian) with random slopes for Condition + nested Trial VC
        pw = mixedlm_pairwise_contrasts(
            dvar_stats.rename(columns={var: "value"}),
            value_col="value",
            group_col="Condition",
            id_col="ID",
            trial_col="Trial",
            add_random_slopes=True,
            p_adjust="bonferroni"
        )

        # === FIGURE (subject-means rainclouds, as before)
        ...
        for cond_lab in condition_order:
            yvals = dvar_plot.loc[dvar_plot["Condition"] == cond_lab, var].dropna().to_numpy()
            if yvals.size == 0:
                continue

            # limits for violin construction
            if var in ylims_map:
                ymin_cap, ymax_cap = ylims_map[var][0], ylims_map[var][1]
            else:
                ymin_cap, ymax_cap = float(np.nanmin(yvals)), float(np.nanmax(yvals))
            ...
            ax.fill(poly_x, poly_y, facecolor=pal_dict[cond_lab], edgecolor="none",
                    alpha=viol_alpha, clip_on=True)

            # DOTS = subject means
            x_jit = xpos[cond_lab] + rng.uniform(-box_width/2, box_width/2, size=yvals.size)
            ax.scatter(x_jit, yvals, s=dot_size, alpha=dot_alpha, color=pal_dict[cond_lab],
                       linewidths=0, zorder=3)

            # BOXPLOT over subject means
            bp = ax.boxplot(
        ...
        if len(condition_order) >= 2:
            ax.annotate(task["xlabel"], xy=(xpos[condition_order[1]], 0),
                        xycoords=("data", "axes fraction"),
                        xytext=(0, -28), textcoords="offset points",
                        ha="center", va="top")

        # --- Brackets using global cap per variable
        dplot_col = dvar_plot[var]
        ymin = float(np.nanmin(dplot_col)) if np.isfinite(np.nanmin(dplot_col)) else 0.0
        ymax_data_local = float(np.nanmax(dplot_col)) if np.isfinite(np.nanmax(dplot_col)) else ymin
        ...
        if var in ylims_map:
            ...
            ymid = (ymin_set + ymax_cap) / 2.0
        else:
            ymid = (ymin + ymax_cap) / 2.0

        ax.set_ylabel("")
        ax.yaxis.get_label().set_visible(False)
        ax.text(-0.12, ymid, ylab, transform=ax.get_yaxis_transform(which='grid'),
        ...
        # small bump for the n-back Accuracy brackets if needed
        ...
        add_stat_brackets(
            ...
            labels=labels,
            xmap={c: i for i, c in enumerate(condition_order)}
        )
        ...
        if var in ylims_map:
            ax.set_ylim(*ylims_map[var])

        # Save
        fig.tight_layout()
        fig.savefig(
            os.path.join(output_dir, f"AOC_stats_rainclouds_{sname}_{task['name']}_stats_trials.png"),
            dpi=300,
            transparent=False,
