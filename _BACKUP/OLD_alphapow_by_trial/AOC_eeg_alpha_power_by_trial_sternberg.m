...
for subj = 1:length(subjects)
    ...
    for trl = 1:num_trials
        ...
        subj_data_eeg_trials = struct('ID', num2cell(subject_id), 'Trial', num2cell(trial_num), 'Condition', num2cell(condition), 'AlphaPower', num2cell(alpha), 'IAF', num2cell(IAF));

        %% Calculate subject-specific AlphaPower by condition
        l2 = subj_data_eeg_trials([subj_data_eeg_trials.Condition] == 2);
        l2alphapow = mean([l2.AlphaPower], 'omitnan');
        l4 = subj_data_eeg_trials([subj_data_eeg_trials.Condition] == 4);
        l4alphapow = mean([l4.AlphaPower], 'omitnan');
        l6 = subj_data_eeg_trials([subj_data_eeg_trials.Condition] == 6);
    ...
    mkdir(savepath)
    cd(savepath)
    save eeg_matrix_sternberg_subj_trial subj_data_eeg_trials
...
save /Volumes/methlab/Students/Arne/AOC/data/features/eeg_matrix_sternberg_trials eeg_data_sternberg_trials