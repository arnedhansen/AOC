...
for subj = 1:length(subjects)
    ...
    cfg = [];
    cfg.channel = {'L-GAZE-X'  'L-GAZE-Y' 'L-AREA'}; % NEEDS TO BE ADJUSTED FOR RIGHT EYE +++ DISTANCE and PUPIL SIZE
    dataet = ft_selectdata(cfg,data);

    %% Get EEG data (excl. ET and EOG data)
    cfg = [];
    cfg.channel = {'all' '-B*' '-HEOGR' '-HEOGL', '-VEOGU', '-VEOGL' ,'-L-GAZE-X' , '-L-GAZE-Y' , '-L-AREA'}; % NEEDS TO BE ADJUSTED FOR RIGHT EYE +++ DISTANCE and PUPIL SIZE
