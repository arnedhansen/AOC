...
try
    ...
    task = [subjectID, '_AOC_Sternberg', '_block7_task', '_EEG.mat'];
    % save to a file
    save(fullfile(filePath, task), 'EEG', '-v7.3')
catch ME
    ME.message
    warning('Block 7 is missing...')
end

% Block 8
try
    EEG = pop_select(EEGorig, 'point', [EEGorig.event(i38(end)).latency, EEGorig.event(i48(end)).latency]);
    task = [subjectID, '_AOC_Sternberg', '_block8_task', '_EEG.mat'];
    % save to a file
    save(fullfile(filePath, task), 'EEG', '-v7.3')
catch ME
    ME.message
    warning('Block 8 is missing...')
end
