load("/Volumes/Homestore/OCC/arne/figuresstage2/MS_GAZE_timecourses/NbackMSDEV.mat")
allDevNback = allDev;
allMSNback  = allMS;
load("/Volumes/Homestore/OCC/arne/figuresstage2/MS_GAZE_timecourses/SternbergMSDEV.mat")
allDevStern = allDev;
allMSStern  = allMS;
%% apply baseline optional, first do without
% for cond = 1:3
%     for s = 1:length(allDevStern{1})
%         cfg = [];
%         cfg.baseline = [-1.5 -.5];
%         allDevNback{cond}{s}= ft_timelockbaseline(cfg,allDevNback{cond}{s});
%         allMSNback{cond}{s}= ft_timelockbaseline(cfg,allMSNback{cond}{s});
%         allDevStern{cond}{s}= ft_timelockbaseline(cfg,allDevStern{cond}{s});
%         allMSStern{cond}{s}= ft_timelockbaseline(cfg,allMSStern{cond}{s});
%     end
% end
%% compute grand average

for cond = 1:3

    cfg = [];
    cfg.keepindividual = 'yes';

    GA_dev_nback{cond} = ft_timelockgrandaverage(cfg, allDevNback{cond}{:});
    GA_ms_nback{cond}  = ft_timelockgrandaverage(cfg, allMSNback{cond}{:});

    GA_dev_stern{cond} = ft_timelockgrandaverage(cfg, allDevStern{cond}{:});
    GA_ms_stern{cond}  = ft_timelockgrandaverage(cfg, allMSStern{cond}{:});
end
%% plot
cfg = [];
cfg.channel = 'GAZEDEV';
cfg.figure ='gcf';
cfg.baseline = [-.5 -.25];
figure;
ft_singleplotER(cfg, GA_dev_stern{1}, GA_dev_stern{2}, GA_dev_stern{3});
%% plot ms rate
cfg = [];
cfg.channel = 'MSRATE';
cfg.figure ='gcf';
% cfg.baseline = [-.5 -.25];
figure;
ft_singleplotER(cfg, GA_ms_stern{1}, GA_ms_stern{2}, GA_ms_stern{3});
%% plot gaze deviation with SEM
close all
figure;
subplot(2,2,1);
cfg = [];
cfg.channel = {'GAZEDEV'};
cfg.latency = [-.5 2];
tlk2_ind        = ft_selectdata(cfg,GA_dev_nback{1});
tlk4_ind        = ft_selectdata(cfg,GA_dev_nback{2});
tlk6_ind        = ft_selectdata(cfg,GA_dev_nback{3});
% plot load 6
x = tlk6_ind.time'; % x-axis def
y = mean(squeeze(tlk6_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk6_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound


hp1 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'r', 'HandleVisibility', 'off');
hold on;
hl1 = line(x, y);
set(hp1, 'facecolor', [0.97, 0.26, 0.26], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl1, 'color', [0.97, 0.26, 0.26], 'linewidth', 2);

% plot load 4
x = tlk4_ind.time'; % x-axis def
y = mean(squeeze(tlk4_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk4_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound

hp2 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'b', 'HandleVisibility', 'off');
hl2 = line(x, y);
set(hp2, 'facecolor', [0.30, 0.75, 0.93], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl2, 'color', [0.30, 0.75, 0.93], 'linewidth', 2);

% plot load 2
x = tlk2_ind.time'; % x-axis def
y = mean(squeeze(tlk2_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk2_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound

hp3 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'k', 'HandleVisibility', 'off');
hl3 = line(x, y);
set(hp3, 'facecolor', [0.5, 0.5, 0.5], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl3, 'color', 'k', 'linewidth', 2);

% Label the axes
set(gca, 'FontSize', 22);
title('N-back task');
xlabel('Time [sec]');
% ylabel("Power change \newline from baseline");
ylabel("Gaze deviation [px]");
set(gcf,'color','w');
box on;
grid on;
legend({'Load 3','Load 2','Load 1'}, 'Location','northwest','Fontsize',18);
xlim([-.5 2])
% ylim([-3 3]);
%% plot sternberg
subplot(2,2,2);
cfg = [];
cfg.channel = {'GAZEDEV'};
cfg.latency = [-.5 2];
tlk2_ind        = ft_selectdata(cfg,GA_dev_stern{1});
tlk4_ind        = ft_selectdata(cfg,GA_dev_stern{2});
tlk6_ind        = ft_selectdata(cfg,GA_dev_stern{3});
% plot load 6
x = tlk6_ind.time'; % x-axis def
y = mean(squeeze(tlk6_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk6_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound


hp1 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'r', 'HandleVisibility', 'off');
hold on;
hl1 = line(x, y);
set(hp1, 'facecolor', [0.97, 0.26, 0.26], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl1, 'color', [0.97, 0.26, 0.26], 'linewidth', 2);

% plot load 4
x = tlk4_ind.time'; % x-axis def
y = mean(squeeze(tlk4_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk4_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound

hp2 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'b', 'HandleVisibility', 'off');
hl2 = line(x, y);
set(hp2, 'facecolor', [0.30, 0.75, 0.93], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl2, 'color', [0.30, 0.75, 0.93], 'linewidth', 2);

% plot load 2
x = tlk2_ind.time'; % x-axis def
y = mean(squeeze(tlk2_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk2_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound

hp3 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'k', 'HandleVisibility', 'off');
hl3 = line(x, y);
set(hp3, 'facecolor', [0.5, 0.5, 0.5], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl3, 'color', 'k', 'linewidth', 2);

% Label the axes
set(gca, 'FontSize', 22);
title('Sternberg task');
xlabel('Time [sec]');
% ylabel("Power change \newline from baseline");
ylabel("Gaze deviation [px]");
set(gcf,'color','w');
box on;
grid on;
legend({'Load 6','Load 4','Load 2'}, 'Location','northwest','Fontsize',18);
xlim([-.5 2])
% ylim([-3 3]);
%% plot ms rate with SEM

subplot(2,2,3);
cfg = [];
cfg.channel = {'MSRATE'};
cfg.latency = [-.5 2];
tlk2_ind        = ft_selectdata(cfg,GA_ms_nback{1});
tlk4_ind        = ft_selectdata(cfg,GA_ms_nback{2});
tlk6_ind        = ft_selectdata(cfg,GA_ms_nback{3});
% plot load 6
x = tlk6_ind.time'; % x-axis def
y = mean(squeeze(tlk6_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk6_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound


hp1 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'r', 'HandleVisibility', 'off');
hold on;
hl1 = line(x, y);
set(hp1, 'facecolor', [0.97, 0.26, 0.26], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl1, 'color', [0.97, 0.26, 0.26], 'linewidth', 2);

% plot load 4
x = tlk4_ind.time'; % x-axis def
y = mean(squeeze(tlk4_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk4_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound

hp2 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'b', 'HandleVisibility', 'off');
hl2 = line(x, y);
set(hp2, 'facecolor', [0.30, 0.75, 0.93], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl2, 'color', [0.30, 0.75, 0.93], 'linewidth', 2);

% plot load 2
x = tlk2_ind.time'; % x-axis def
y = mean(squeeze(tlk2_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk2_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound

hp3 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'k', 'HandleVisibility', 'off');
hl3 = line(x, y);
set(hp3, 'facecolor', [0.5, 0.5, 0.5], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl3, 'color', 'k', 'linewidth', 2);

% Label the axes
set(gca, 'FontSize', 22);
title('');
xlabel('Time [sec]');
% ylabel("Power change \newline from baseline");
ylabel("MS rate [Hz]");
set(gcf,'color','w');
box on;
grid on;
legend({'Load 3','Load 2','Load 1'}, 'Location','southeast','Fontsize',18);
xlim([-.5 2])
% ylim([-3 3]);
%% plot sternberg ms rate
subplot(2,2,4);
cfg = [];
cfg.channel = {'MSRATE'};
cfg.latency = [-.5 2];
tlk2_ind        = ft_selectdata(cfg,GA_ms_stern{1});
tlk4_ind        = ft_selectdata(cfg,GA_ms_stern{2});
tlk6_ind        = ft_selectdata(cfg,GA_ms_stern{3});
% plot load 6
x = tlk6_ind.time'; % x-axis def
y = mean(squeeze(tlk6_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk6_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound


hp1 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'r', 'HandleVisibility', 'off');
hold on;
hl1 = line(x, y);
set(hp1, 'facecolor', [0.97, 0.26, 0.26], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl1, 'color', [0.97, 0.26, 0.26], 'linewidth', 2);

% plot load 4
x = tlk4_ind.time'; % x-axis def
y = mean(squeeze(tlk4_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk4_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound

hp2 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'b', 'HandleVisibility', 'off');
hl2 = line(x, y);
set(hp2, 'facecolor', [0.30, 0.75, 0.93], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl2, 'color', [0.30, 0.75, 0.93], 'linewidth', 2);

% plot load 2
x = tlk2_ind.time'; % x-axis def
y = mean(squeeze(tlk2_ind.individual), 1)'; % y-axis def
e = std(squeeze(tlk2_ind.individual), 1)' ./ sqrt(numel(tlk6_ind.individual(:,1,1))); % sme
low = y - e; % lower bound
high = y + e; % upper bound

hp3 = patch([x; x(end:-1:1); x(1)], [low; high(end:-1:1); low(1)], 'k', 'HandleVisibility', 'off');
hl3 = line(x, y);
set(hp3, 'facecolor', [0.5, 0.5, 0.5], 'edgecolor', 'none', 'facealpha', 0.2);
set(hl3, 'color', 'k', 'linewidth', 2);

% Label the axes
set(gca, 'FontSize', 22);
title('');
xlabel('Time [sec]');
ylabel("MS rate [Hz]");
set(gcf,'color','w');
box on;
grid on;
legend({'Load 6','Load 4','Load 2'}, 'Location','northeast','Fontsize',18);
xlim([-.5 2])
% ylim([-3 3]);
%% extract gazedev values
cfg =[];
cfg.channel = {'GAZEDEV'};
cfg.latency = [0 2];
cfg.avgovertime = 'yes';
val_nb_1 = ft_selectdata(cfg,GA_dev_nback{1});
val_nb_2 = ft_selectdata(cfg,GA_dev_nback{2});
val_nb_3 = ft_selectdata(cfg,GA_dev_nback{3});

val_nb_1 = val_nb_1.individual;
val_nb_2 = val_nb_2.individual;
val_nb_3 = val_nb_3.individual;

cfg =[];
cfg.channel = {'GAZEDEV'};
cfg.latency = [1 2];
cfg.avgovertime = 'yes';

val_sb_2 = ft_selectdata(cfg,GA_dev_stern{1});
val_sb_4 = ft_selectdata(cfg,GA_dev_stern{2});
val_sb_6 = ft_selectdata(cfg,GA_dev_stern{3});
val_sb_2 = val_sb_2.individual;
val_sb_4 = val_sb_4.individual;
val_sb_6 = val_sb_6.individual;
%% plot distributions nback
figure; clf;
subplot(1,2,1);
positions = [.3, .6, .9];  % Adjusted x-positions

% Kernel Density Plots
[f_sb2, xi_sb2] = ksdensity(val_nb_1);
fill(positions(1) + f_sb2*0.4, xi_sb2, 'k', 'FaceAlpha', 0.5); hold on;

[f_sb4, xi_sb4] = ksdensity(val_nb_2);
fill(positions(2) + f_sb4*0.4, xi_sb4, 'b', 'FaceAlpha', 0.5); hold on;

[f_sb6, xi_sb6] = ksdensity(val_nb_3);
fill(positions(3) + f_sb6*0.4, xi_sb6, 'r', 'FaceAlpha', 0.5); hold on;

% Boxplots
box_h = boxplot([val_nb_1(:), val_nb_2(:), val_nb_3(:)], ...
    'Labels', {'load 1', 'load 2', 'load 3'}, ...
    'Widths', 0.05, ...
    'Positions', positions);
set(box_h, 'LineWidth', 2);

% Raindrop scatter
jitter = 0.05;
scatter(positions(1) + (rand(size(val_nb_1))-0.5)*jitter, val_nb_1, 'k.');
scatter(positions(2) + (rand(size(val_nb_2))-0.5)*jitter, val_nb_2, 'b.');
scatter(positions(3) + (rand(size(val_nb_3))-0.5)*jitter, val_nb_3, 'r.');


ylabel('gaze deviation ');
xlim([0 1]);
title('');
box on;
set(gcf,'color','w');
set(gca,'Fontsize',20);

% Significance tests
[~, p_24] = ttest(val_nb_1, val_nb_2);
[~, p_46] = ttest(val_nb_2, val_nb_3);
[~, p_26] = ttest(val_nb_1, val_nb_3);

% Significance annotations
y_max = max([val_nb_1(:); val_nb_2(:); val_nb_3(:)]) + 2;  % smaller margin above data overall height above
y_step = 0.5;  % tighter vertical spacing between lines

sig_label = getSigLabel(p_24);
if ~isempty(sig_label)
    line([positions(1), positions(2)], [y_max, y_max], 'Color', 'k', 'LineWidth', 1.2);
    text(mean([positions(1), positions(2)]), y_max + 0.2, sig_label, ...
        'FontSize', 14, 'HorizontalAlignment', 'center');
    y_max = y_max + y_step;
end

sig_label = getSigLabel(p_46);
if ~isempty(sig_label)
    line([positions(2), positions(3)], [y_max, y_max], 'Color', 'k', 'LineWidth', 1.2);
    text(mean([positions(2), positions(3)]), y_max + 0.2, sig_label, ...
        'FontSize', 14, 'HorizontalAlignment', 'center');
    y_max = y_max + y_step;
end

sig_label = getSigLabel(p_26);
if ~isempty(sig_label)
    line([positions(1), positions(3)], [y_max, y_max], 'Color', 'k', 'LineWidth', 1.2);
    text(mean([positions(1), positions(3)]), y_max + 0.2, sig_label, ...
        'FontSize', 14, 'HorizontalAlignment', 'center');
end
% ylim([min([val_nb_1(:); val_nb_2(:); val_nb_3(:)]) - 2, y_max + 2]);
% ylim([-8 10])
xlim([.1 1.1])
%% compute linear mixed model nback

Y = [val_nb_1(:); val_nb_2(:); val_nb_3(:)];

Load = [ones(numel(val_nb_1),1); ...
        2*ones(numel(val_nb_2),1); ...
        3*ones(numel(val_nb_3),1)];

Subj = [(1:numel(val_nb_1))'; ...
        (1:numel(val_nb_2))'; ...
        (1:numel(val_nb_3))'];

Tlong = table(Y, categorical(Load), categorical(Subj), ...
    'VariableNames', {'AlphaPower','Load','Subj'});


lme = fitlme(Tlong, ...
    'AlphaPower ~ Load + (1|Subj)');


disp(lme)
%% simple t-test
[H,P,C,stat]=ttest(val_nb_1, val_nb_2)
[H,P,C,stat]=ttest(val_nb_2, val_nb_3)
[H,P,C,stat]=ttest(val_nb_1, val_nb_3)
%% plot distributions sternberg
% figure; clf;
subplot(1,2,2);
positions = [.3, .6, .9];  % Adjusted x-positions

% Kernel Density Plots
[f_sb2, xi_sb2] = ksdensity(val_sb_2);
fill(positions(1) + f_sb2*0.4, xi_sb2, 'k', 'FaceAlpha', 0.5); hold on;

[f_sb4, xi_sb4] = ksdensity(val_sb_4);
fill(positions(2) + f_sb4*0.4, xi_sb4, 'b', 'FaceAlpha', 0.5); hold on;

[f_sb6, xi_sb6] = ksdensity(val_sb_6);
fill(positions(3) + f_sb6*0.4, xi_sb6, 'r', 'FaceAlpha', 0.5); hold on;

% Boxplots
box_h = boxplot([val_sb_2(:), val_sb_4(:), val_sb_6(:)], ...
    'Labels', {'load 2', 'load 4', 'load 6'}, ...
    'Widths', 0.05, ...
    'Positions', positions);
set(box_h, 'LineWidth', 2);

% Raindrop scatter
jitter = 0.05;
scatter(positions(1) + (rand(size(val_nb_1))-0.5)*jitter, val_sb_2, 'k.');
scatter(positions(2) + (rand(size(val_nb_2))-0.5)*jitter, val_sb_4, 'b.');
scatter(positions(3) + (rand(size(val_nb_3))-0.5)*jitter, val_sb_6, 'r.');


ylabel('\alpha power [dB] ');
xlim([0 1]);
title('');
box on;
set(gcf,'color','w');
set(gca,'Fontsize',20);

% Significance tests
[~, p_24] = ttest(val_sb_2, val_sb_4);
[~, p_46] = ttest(val_sb_4, val_sb_6);
[~, p_26] = ttest(val_sb_2, val_sb_6);

% Significance annotations
y_max = max([val_sb_2(:); val_sb_4(:); val_sb_6(:)]) + 2;  % smaller margin above data overall height above
y_step = 0.5;  % tighter vertical spacing between lines

sig_label = getSigLabel(p_24);
if ~isempty(sig_label)
    line([positions(1), positions(2)], [y_max, y_max], 'Color', 'k', 'LineWidth', 1.2);
    text(mean([positions(1), positions(2)]), y_max + 0.2, sig_label, ...
        'FontSize', 14, 'HorizontalAlignment', 'center');
    y_max = y_max + y_step;
end

sig_label = getSigLabel(p_46);
if ~isempty(sig_label)
    line([positions(2), positions(3)], [y_max, y_max], 'Color', 'k', 'LineWidth', 1.2);
    text(mean([positions(2), positions(3)]), y_max + 0.2, sig_label, ...
        'FontSize', 14, 'HorizontalAlignment', 'center');
    y_max = y_max + y_step;
end

sig_label = getSigLabel(p_26);
if ~isempty(sig_label)
    line([positions(1), positions(3)], [y_max, y_max], 'Color', 'k', 'LineWidth', 1.2);
    text(mean([positions(1), positions(3)]), y_max + 0.2, sig_label, ...
        'FontSize', 14, 'HorizontalAlignment', 'center');
end
% ylim([min([val_nb_1(:); val_nb_2(:); val_nb_3(:)]) - 2, y_max + 2]);
% ylim([-8 10])
xlim([.1 1.1])
%% compute linear mixed model nback

Y = [val_sb_2(:); val_sb_4(:); val_sb_6(:)];

Load = [ones(numel(val_sb_2),1); ...
        2*ones(numel(val_sb_4),1); ...
        3*ones(numel(val_sb_6),1)];

Subj = [(1:numel(val_sb_2))'; ...
        (1:numel(val_sb_4))'; ...
        (1:numel(val_sb_6))'];

Tlong = table(Y, categorical(Load), categorical(Subj), ...
    'VariableNames', {'AlphaPower','Load','Subj'});


lme = fitlme(Tlong, ...
    'AlphaPower ~ Load + (1|Subj)');
disp(lme)
%% simple t-test
[H,P,C,stat]=ttest(val_sb_2, val_sb_4)
[H,P,C,stat]=ttest(val_sb_4, val_sb_6)
[H,P,C,stat]=ttest(val_sb_2, val_sb_6)
%% helper function
function sig_label = getSigLabel(p)
    if p < 0.001
        sig_label = '***';
    elseif p < 0.01
        sig_label = '**';
    elseif p < 0.05
        sig_label = '*';
    else
        sig_label = '';
    end
end

