%% AOC Omnibus — Alpha Power × SPL: Continuous & Median Split Analysis
% Analyzes the relationship between alpha power and scan path length for both
% n-back and Sternberg tasks using two approaches:
% 1. Continuous correlation: Time-resolved correlation between alpha power and SPL
...
for task_idx = 1:length(tasks)
    ...
    spl_trajectories = nan(n_subj, Tf);
    
    %% Load alpha power and gaze data
    valid_subj_count = 0;
    mean_alpha_valid = [];
    spl_trajectories_valid = [];
    ...
    for s = 1:n_subj
        ...
        if ~exist('ScanPathSeries', 'var') || ~exist('ScanPathSeriesT', 'var')
            n_load_errors = n_load_errors + 1;
            continue;
        end
        
        if isempty(ScanPathSeries) || isempty(ScanPathSeriesT)
        ...
        for trl = 1:numel(ScanPathSeries)
            ...
            % Check dimensions
            if numel(srl_full) ~= numel(tt_full)
                continue;
            end
            
            try
                interp_vals = interp1(tt_full, srl_full, t_plot_full, 'linear', NaN);
        ...
        if valid_trials > 0
            ...
            if n_valid_points > 100  % At least 100 time points with data
                ...
                spl_trajectories_valid(valid_subj_count, :) = subj_mean;
            else
    ...
    spl_trajectories = spl_trajectories_valid;
    ...
    for t = 1:Tf
        spl_vals = spl_trajectories(:, t);
        valid = isfinite(mean_alpha) & isfinite(spl_vals);
        if sum(valid) >= 3
            [r, p] = corrcoef(mean_alpha(valid), spl_vals(valid));
            r_vals(t) = r(1, 2);
            p_vals(t) = p(1, 2);
        end
    end
    ...
    % Compute group means
    spl_high = spl_trajectories(high_reduction_mask, :);
    spl_low = spl_trajectories(low_reduction_mask, :);
    
    grand_high = nanmean(spl_high, 1);
    grand_low = nanmean(spl_low, 1);
    sem_high = nanstd(spl_high, [], 1) ./ sqrt(sum(isfinite(spl_high), 1));
    sem_low = nanstd(spl_low, [], 1) ./ sqrt(sum(isfinite(spl_low), 1));
    ...
    for t = 1:T
        [~, idx] = min(abs(t_plot_full - t_plot(t)));
        high_vals = spl_high(:, idx);
        low_vals = spl_low(:, idx);
    ...
    title(sprintf('%s: Time-Resolved Correlation (Alpha Power × SPL)', upper(task_name)), ...
    ...
    % Save figure
    fig_name = sprintf('AOC_omnibus_alpha_SPL_%s.png', task_name);
