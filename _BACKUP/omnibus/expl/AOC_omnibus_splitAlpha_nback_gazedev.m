%% AOC Omnibus — N-Back: Split by Alpha Power Change (Gaze Deviation)
...
for analysis_idx = 1:length(all_analyses)
    ...
    if length(reduction_ids) < 2 || length(amplification_ids) < 2
        warning('Not enough subjects for %s. Skipping.', analysis_name);
        continue;
    end
    
    %% Load gaze data and compute gaze deviation
    ...
    if ~isempty(missing_gaze)
        fprintf('  Warning: Missing gaze data for %d subjects\n', length(missing_gaze));
    end
    ...
    title(sprintf('N-Back Gaze Deviation: Alpha Reduction vs Amplification (%s)', analysis_name), ...
    ...
    fig_name = sprintf('AOC_omnibus_splitAlpha_nback_gazedev_%s.png', cond_label);
