%% AOC Omnibus — Sternberg: Split by Alpha Power Change (Eye Velocity)
% Loads omnibus_data_rainclouds.csv, splits Sternberg participants by alpha power change
% (reduction vs amplification) per condition and overall. Shows time-resolved eye velocity
% for each group with statistical comparisons. Saves figures.
%
% Key outputs:
%   Eye velocity time-series figures (reduction vs amplification) per condition and overall
...
for analysis_idx = 1:length(all_analyses)
    ...
    %% Load gaze data and compute velocity time series
    % Preallocate holders
    vel_reduction = nan(length(subjects), Tf);
    vel_amplification = nan(length(subjects), Tf);
    ...
    for subj = 1:length(subjects)
        ...
        % Compute velocity for each trial and interpolate to common grid
        ...
        if is_reduction
            ...
            vel_reduction(reduction_subj_count, :) = subj_mean;
        end
        
        if is_amplification
            amplification_subj_count = amplification_subj_count + 1;
            vel_amplification(amplification_subj_count, :) = subj_mean;
        end
    ...
    vel_reduction = vel_reduction(1:reduction_subj_count, :);
    vel_amplification = vel_amplification(1:amplification_subj_count, :);
    ...
    for t = 1:Tf
        red_vals = vel_reduction(:, t);
        amp_vals = vel_amplification(:, t);
    ...
    vel_reduction_binned = nan(size(vel_reduction, 1), T);
    vel_amplification_binned = nan(size(vel_amplification, 1), T);
    
    for s = 1:size(vel_reduction, 1)
        vel_reduction_binned(s, :) = interp1(t_plot_full, vel_reduction(s, :), ...
            time_series(2:end), 'linear', NaN);
    end
    
    for s = 1:size(vel_amplification, 1)
        vel_amplification_binned(s, :) = interp1(t_plot_full, vel_amplification(s, :), ...
            time_series(2:end), 'linear', NaN);
    end
    
    % Grand averages and SEM (binned)
    grand_reduction = nanmean(vel_reduction_binned, 1);
    grand_amplification = nanmean(vel_amplification_binned, 1);
    sem_reduction = nanstd(vel_reduction_binned, [], 1) ./ sqrt(sum(isfinite(vel_reduction_binned), 1));
    sem_amplification = nanstd(vel_amplification_binned, [], 1) ./ sqrt(sum(isfinite(vel_amplification_binned), 1));
    t_plot = time_series(2:end);
    
    % Cohen's d for binned data (for plotting)
    d_vals_binned = nan(1, T);
    for t = 1:T
        red_vals = vel_reduction_binned(:, t);
        amp_vals = vel_amplification_binned(:, t);
        red_vals = red_vals(isfinite(red_vals));
        amp_vals = amp_vals(isfinite(amp_vals));
    ...
    title(sprintf('Sternberg Eye Velocity: Alpha Reduction vs Amplification (%s)', analysis_name), ...
    ...
    % Save figure
    fig_name = sprintf('AOC_omnibus_splitAlpha_velocity_%s.png', cond_label);
