...
% `AOC_eeg_matrix_nback_FOOOF.mat` (repeated per trial by ID/Condition).
% Produces merged_data_nback_trials.mat.
%
% Key outputs:
...
T_fooof = struct2table(eeg_data_nback_FOOOF);
assert_unique_keys(T_fooof, {'ID', 'Condition'}, 'eeg_data_nback_FOOOF');

for i = 1:height(T_fooof)
    rowIdx = merged_data_nback_trials.ID == T_fooof.ID(i) & ...
        merged_data_nback_trials.Condition == T_fooof.Condition(i);
