# AOC Power Analysis — Sternberg Condition (SIMR)
# Simulates power for mixed model (alpha ~ condition + (1|Subject)) with simr; effect size d=0.19 (Hu et al. 2019). Power curve over N=5–125. Saves figure.
#
# Key outputs:
#   AOC_power_analysis_sternberg.png; power curve (console)
...
write.csv(
  power_curve_df,
  file.path(fig_dir, "AOC_power_analysis_sternberg.csv"),
  row.names = FALSE
)
save_power_curve_plot(
  power_curve_df,
  file.path(fig_dir, "AOC_power_analysis_sternberg.png")
)