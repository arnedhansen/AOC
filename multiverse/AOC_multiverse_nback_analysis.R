...
#   multiverse_nback_conditions_results.csv    — per-condition gaze→alpha
#   multiverse_nback_condition_results.csv     — condition→alpha (EEG-only)
#   multiverse_nback_interaction_results.csv   — gaze×condition interaction term
#   multiverse_nback_condition_gaze_results.csv — condition→gaze (gaze-only)
...
csv_path_method <- file.path(csv_dir, paste0("multiverse_nback_", FOOOF_METHOD, ".csv"))
csv_path <- if (file.exists(csv_path_method)) csv_path_method else file.path(csv_dir, "multiverse_nback.csv")
...
write.csv(
  ...
  file.path(csv_dir, "multiverse_nback_fooof_trial_r2_filter_summary.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(csv_dir, "multiverse_nback_fooof_trial_r2_filter_summary_by_latency.csv"),
  row.names = FALSE
)
...
save_with_method(M_int, "multiverse_nback_results.csv")
save_with_method(M_cond, "multiverse_nback_conditions_results.csv")
message("Saved: multiverse_nback_results.csv, multiverse_nback_conditions_results.csv")
...
if (nrow(M_interaction) > 0) {
  save_with_method(M_interaction, "multiverse_nback_interaction_results.csv")
  message(sprintf("Saved: multiverse_nback_interaction_results.csv (%d universes)", nrow(M_interaction)))
} else {
...
if (nrow(M_ca) > 0L) {
  M_ca <- add_sig(M_ca)
  save_with_method(M_ca, "multiverse_nback_condition_results.csv")
  message(sprintf("Saved: multiverse_nback_condition_results.csv (%d universes)", nrow(M_ca)))
} else {
...
if (nrow(M_cg) > 0L) {
  M_cg <- add_sig(M_cg)
  save_with_method(M_cg, "multiverse_nback_condition_gaze_results.csv")
  message(sprintf("Saved: multiverse_nback_condition_gaze_results.csv (%d universes)", nrow(M_cg)))
} else {
...
if ("aperiodic_offset" %in% names(dat) && "aperiodic_exponent" %in% names(dat)) {
  ...
  if (nrow(M_ap_gaze_results) > 0) {
    ...
    save_with_method(M_ap_gaze_results, "multiverse_nback_aperiodic_gaze_results.csv")
    message(sprintf("Saved: multiverse_nback_aperiodic_gaze_results.csv (%d rows)", nrow(M_ap_gaze_results)))
  }
  ...
  if (nrow(M_ap_cond_results) > 0) {
    ...
    save_with_method(M_ap_cond_results, "multiverse_nback_aperiodic_condition_results.csv")
    message(sprintf("Saved: multiverse_nback_aperiodic_condition_results.csv (%d rows)", nrow(M_ap_cond_results)))
  }
