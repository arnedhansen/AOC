...
#
# Key difference from trial-level: robust_z is applied to subject means
# (not individual trials), eliminating potential within-subject distortions.
#
# Models:
...
inside(M, {
  ...
  # Per-condition: use lm() since subject means yield 1 row per subject per condition
  tid_cond <- if (valid) {
