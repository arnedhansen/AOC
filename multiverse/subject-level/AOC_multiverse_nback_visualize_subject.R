...
#
# Figures (identical layout to trial-level, with _subject suffix):
...
#   6 (_subject_aperiodic):       aperiodic component [combined forest plot]
#
...
ggsave(file.path(storage_plot, "AOC_multiverse_nback_subject_estimate.png"),
       plot = p_combined, width = 14, height = 12, dpi = 600)
message("Saved: AOC_multiverse_nback_subject_estimate.png")
...
# Top: aperiodic ~ gaze — forest plot grouped by gaze measure (4 × 16 universes)
...
ap_cond_path <- file.path(csv_dir, "multiverse_nback_subject_aperiodic_condition_results.csv")
...
if (has_ap_gaze || has_ap_cond) {
  ...
  if (has_ap_gaze) {
    ...
    M_ap_gaze <- M_ap_gaze %>%
      filter(gaze_measure %in% gaze_order) %>%
      mutate(
        gaze_label = rename_opts(gaze_measure),
        gaze_label = factor(gaze_label, levels = c("Gaze Deviation", "Gaze Velocity", "Scan Path Length", "BCEA")),
    ...
    p_forest_gaze <- ggplot(M_ap_gaze, aes(x = estimate, y = rank, color = condition)) +
      ...
      labs(title = expression(bold("Aperiodic ~ gaze (subject-level)")),
  ...
  if (has_ap_cond) {
    ...
    p_forest_cond <- ggplot(M_ap_cond, aes(x = estimate, y = universe_label, color = condition)) +
      ...
      labs(title = expression(bold("Aperiodic ~ condition (subject-level)")),
  ...
  ggsave(file.path(storage_plot, "AOC_multiverse_nback_subject_aperiodic.png"),
         plot = p_ap_combined, width = 12, height = 14, dpi = 600)
  message("Saved: AOC_multiverse_nback_subject_aperiodic.png")

} else {
  message("Skipping Figure 6: no aperiodic result CSVs found.")
}

# ========== FIGURE 7: APERIODIC ~ GAZE — SPECIFICATION CURVES ==========
...
if (has_ap_gaze) {
  ...
  message("Skipping Figure 7: aperiodic gaze CSV not found.")
}
