%% AOC Split ERS/ERD (Trial-Level, Inverse) — MS (Sternberg + N-back)
% Within-subject median split on microsaccade rate; outcome = occipital ERSD.
...
split_file = fullfile(stats_dir, 'AOC_splitMS_inverse_trialSplits.mat');
if ~isfile(split_file)
    error('Missing inverse split file. Run AOC_splitBehavior_inverse_Prep.m first:\n  %s', split_file);
end
Ssplit = load(split_file, 'split_ms_inverse');
split_data = Ssplit.split_ms_inverse;

fprintf('\n=== AOC Split ERS/ERD Inverse — Microsaccades (trial-level) ===\n');
...
tasks(1).group_lbl_low = 'Low MS';
tasks(1).group_lbl_high = 'High MS';
tasks(1).split_var = 'MSRateFullBL';
tasks(1).ersd_var = 'ERSD_late';

tasks(2).tag = 'nback';
tasks(2).split_label = 'splitMedian_trial';
tasks(2).group_lbl_low = 'Low MS';
tasks(2).group_lbl_high = 'High MS';
tasks(2).split_var = 'MSRateFullBL';
...
fig_prefix = sprintf('AOC_splitERSD_%s_MS_inverse', task_tag);
...
ylabel('Mean trial MS rate change [% BL]');
...
fprintf('\n=== Aggregating ERSD within MS trial-split groups (%d subjects) ===\n', nSubj);
for s = 1:nSubj
    clc; fprintf('[SPLIT INVERSE MS - %s] Subject %d / %d\n', upper(task_tag), s, nSubj);
    metrics_MS(s, 1) = split_mean_low(s);
    metrics_MS(s, 2) = split_mean_high(s);
...
plot_paired_raincloud(metrics_MS(:, 1), metrics_MS(:, 2), colors, ...
    tk.group_lbl_low, tk.group_lbl_high, 'Microsaccade rate [% BL]', ...
    fig_dir_root, sprintf('%s_raincloud_ms.png', fig_prefix), fig_pos, fontSize);
plot_paired_raincloud(metrics_ERSD(:, 1), metrics_ERSD(:, 2), colors, ...
    tk.group_lbl_low, tk.group_lbl_high, 'ERSD [dB]', ...
    fig_dir_root, sprintf('%s_raincloud_ersd.png', fig_prefix), fig_pos, fontSize);
...
cbpt_report_file = fullfile(stats_dir, sprintf('AOC_splitERSD_MS_inverse_%s_%s_CBPT_report.txt', task_tag, split_label));
init_cbpt_report_file(cbpt_report_file, struct( ...
    'script', 'AOC_splitERSD_MS_inverse.m', ...
    'task_tag', task_tag, ...
...
tc_base = sprintf('AOC_splitERSD_%s_MS_inverse_timecourse', task_tag);
report_tag = sprintf('%s_%s_MS_inverse', task_tag, split_label);
...
for s = 1:nSubj
    ...
    for g = 1:2
        ...
        if incl
            ...
            if g == 1
                ERSDSummary_col(end+1, 1) = ersd_sum_low(ix);
            else
                ERSDSummary_col(end+1, 1) = ersd_sum_high(ix);
            end
        else
...
stats_tbl = table(ID_col, Group_col, Included_col, MS_col, ERSD_col, ERSDSummary_col, ...
    'VariableNames', {'ID', 'Group', 'Included', tk.split_var, tk.ersd_var, 'ERSD_tc_0_2s'});
csv_out = fullfile(stats_dir, sprintf('AOC_splitERSD_MS_inverse_%s_%s_stats_input.csv', task_tag, split_label));
...
report_cfg = struct('tag', report_tag, 'modality', 'ERSD', 'nR', n_pairs, 'n_input', num_subjects_in, ...
