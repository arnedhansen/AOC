...
if win_sm > 1, low_curves = movmean(low_curves, win_sm, 2, 'omitnan'); high_curves = movmean(high_curves, win_sm, 2, 'omitnan'); end
...
log_cbpt_report(cbpt_report_file, build_cbpt_report_lines(struct( ...
    'tag', report_tag, 'modality', 'gaze', 'nR', num_subjects, 'nA', num_subjects, 'lbl_low', lblLow, 'lbl_high', lblHigh, ...
...
nS = size(low_group_timecourses,1); nT = size(low_group_timecourses,2);
df = nS - 1;
for t = 1:nT
    low_values = low_group_timecourses(:,t); high_values = high_group_timecourses(:,t); paired_ok = isfinite(low_values)&isfinite(high_values);
    if ~any(paired_ok), low_values(:)=0; high_values(:)=0;
    else, low_mean=mean(low_values(paired_ok)); high_mean=mean(high_values(paired_ok)); low_values(~paired_ok)=low_mean; high_values(~paired_ok)=high_mean; end
...
tl2 = tl1; tl1.trial = reshape(low_group_timecourses,[nS,1,nT]); tl2.trial = reshape(high_group_timecourses,[nS,1,nT]);
